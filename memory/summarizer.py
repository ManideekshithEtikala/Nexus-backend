    ...
    async def summarize_batch(self, messages: list[BaseMessage]) -> str:
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """Create a concise bullet-point summary of this conversation segment.
...
OUTPUT ONLY bullet points — no explanations, no headings, no meta-commentary, no tags.""",
    ...
    async def update_summary(
        self, old_summary: str, new_messages: list[BaseMessage]
    ) -> str:
        ...
        prompt = ChatPromptTemplate.from_messages(
            [
                (
                    "system",
                    """Update the existing summary by incorporating NEW information.
RULES:
...
7. Output ONLY bullet points — no explanations, no metadata, no tags.""",
        ...
        result = await chain.ainvoke(
            {"old_summary": old_summary, "new_messages": new_text}
        )
        return result.content.strip()
