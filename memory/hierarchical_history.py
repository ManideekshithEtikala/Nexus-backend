...
class HierarchicalSQLChatMessageHistory:
    """
    ...
    async def aget_messages(self) -> list[BaseMessage]:
        ...
        print(f"[Hierarchical] Session {self.session_id}: Total messages = {len(all_messages)}")
        
        if len(all_messages) <= SHORT_TERM_WINDOW:
            print(f"[Hierarchical] All messages fit in short-term window, no summary needed")
            return all_messages
        ...
        if summary:
            ...
            print(f"[Hierarchical] Returning {len(result)} messages (1 summary + {len(recent)} raw)")
            return result
